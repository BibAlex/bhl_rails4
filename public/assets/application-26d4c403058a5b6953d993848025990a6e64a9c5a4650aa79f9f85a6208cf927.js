...
(function($) {
	$.fn.jRating = function(op) {
		var defaults = {
			/** String vars **/
			bigStarsPath : '/assets/images_en/stars.png', // path of the icon stars.png
			smallStarsPath : "/assets/images_en/small.png", // path of the icon small.png
...
;
function confirmDeleteCollectionphoto(id){
    var r= confirm("Are you sure you want to delete collection photo?");
    if (r)
      $.ajax({
        url: "/get_or_delete_collection_photo?id="+id+"&is_delete=1",
        type: "GET",
        success: function (data) {
          $("#photo").html(data);
        }
...
$(document).ready(function(){            
  $(".starsidelink_gray").jRating({     
    isDisabled : true,
    bigStarsPath: "/assets/images_en/stars_grey.png"
  });
});
...
function prepare_avg_rate()
{
	$(".averagerate").jRating({
      isDisabled : true,
      bigStarsPath: "/assets/images_en/stars_white.png"
    });
}
...
function confirmDelete(text){
  ...
  function confirmDeletePhoto(user_id){
    var is_delete = confirm("Are you sure you want to delete your profile photo?");
    if (is_delete)
      load_user_photo(1, user_id);
    else{
      $('#delete_photo').removeAttr('checked');
      return false;
      }
...
function load_user_photo(is_delete, id)
{
	$.ajax({
        url: "/users/" + id + "/get_user_profile_photo?is_delete=" + is_delete,
        type: "GET",        
        success: function (data) { 
          $("#user_photo").html(data);        
        }
      });
}
;
$(document).ready(function(){
  $(".starsidelink_white").jRating({
    isDisabled : true,
    bigStarsPath: "/assets/images_en/stars_white.png"
  });
});
