  ...
  describe "search_volumes" do
    ...
    context "when volumes are sorted by number of views" do
      it "returns top volumes sorted by number of views in DESC order" do
        expect(search_volumes("*:*", 1, MOST_VIEWED_BOOKS, "views desc").first[:job_id]).to eq(2)
        expect(search_volumes("*:*", 1, MOST_VIEWED_BOOKS, "views desc").last[:job_id]).to eq(1)
      end
      
      describe "search volume entry" do
        
        it "returns subjects of volume" do
          expect(search_volumes("*:*", 1, MOST_VIEWED_BOOKS, "views desc").first[:subject]).to eq(["subject_2"])
        end
        
        it "returns title of volume" do
          expect(search_volumes("*:*", 1, MOST_VIEWED_BOOKS, "views desc").first[:title]).to eq(["title_2"])
        end
        
        it "returns authors of volume" do
          expect(search_volumes("*:*", 1, MOST_VIEWED_BOOKS, "views desc").first[:author]).to eq(["author_2"])
        end
        
        it "returns names of volume" do
          expect(search_volumes("*:*", 1, MOST_VIEWED_BOOKS, "views desc").first[:sci_names]).to eq(["sci_name_1", "sci_name_2"])
        end        
      end      
    end
    ...
    context "when volumes are sorted by rate" do
      it "returns top volumes sorted by number rate in DESC order" do
        expect(search_volumes("*:*", 1, MOST_VIEWED_BOOKS, "rate desc").first[:job_id]).to eq(1)
        expect(search_volumes("*:*", 1, MOST_VIEWED_BOOKS, "rate desc").last[:job_id]).to eq(2)
      end
      
      describe "search volume entry" do
        
        it "returns subjects of volume" do
          expect(search_volumes("*:*", 1, MOST_VIEWED_BOOKS, "rate desc").first[:subject]).to eq(["subject_1"])
        end
        
        it "returns title of volume" do
          expect(search_volumes("*:*", 1, MOST_VIEWED_BOOKS, "rate desc").first[:title]).to eq(["title_1"])
        end
        
        it "returns authors of volume" do
          expect(search_volumes("*:*", 1, MOST_VIEWED_BOOKS, "rate desc").first[:author]).to eq(["author_1"])
        end
        
        it "returns names of volume" do
          expect(search_volumes("*:*", 1, MOST_VIEWED_BOOKS, "rate desc").first[:sci_names]).to eq(["sci_name_1_1"])
        end        
      end      
    end
